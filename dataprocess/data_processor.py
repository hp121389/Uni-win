...
class UniRelDataProcessor(object):
    def __init__(self,
        ...
        self._get_labels()  # 将训练集中包含的所有关系类型作为map，赋给label to id,id to label则是相反的map
        if dataset_name == "nyt":
    ...
    def _pre_process(self, path, token_len, is_predict, data_nums):
        ...
        max_token_len = 0
        max_data_nums = math.inf if data_nums == -1 else data_nums # 如果 data_nums = -1，则将 max_data_nums 设置为无穷大；如果不等于 -1，则 max_data_nums 等于 data_nums 的值。这通常用于设置一个数据处理的上限，其中 -1 被用作一个标志值，表示没有上限。
        data_count = 0
        data = json.load(open(path))
        label_dict = {}  # 保存遇到的所有关系类型
        for line in tqdm(data):
            ...
            if token_encode_len > token_len + 2:
                continue
            spo_list = set()  # 遇到的所有三元组的集合
            spo_span_list = set()
            # [CLS] texts [SEP] rels
            # [CLS]: "Classification"的缩写，表示分类任务的起始标记。在BERT模型中，这个标记被用在输入的最前面，模型被训练用这个标记来理解整个句子或文本片段的含义
            # [SEP]: "Separator"的缩写，用来分隔给定的两个句子或文本片段。
            head_matrix = np.zeros([token_len + 2 + self.num_rels,
                                    token_len + 2 + self.num_rels])  # 三个表，分别表示头尾实体的start之间关系，头尾实体的end之间的关系，以及两个实体各自的start和end之间的关系
            tail_matrix = np.zeros(
                [token_len + 2 + self.num_rels, token_len + 2 + self.num_rels])  # num_rels 表示数据集中所有关系的数量
            ...
            spo_text_set = set()
            for spo in line["relation_list"]:  # 这里的spo表示relation_list里的一个relation项
                pred = spo["predicate"]
                if pred not in label_dict:  # 如果关系不在dict里，新建
                    label_dict[pred] = 0
                label_dict[pred] += 1  # 计数
                sub = spo["subject"]
                obj = spo["object"]
                spo_list.add((sub, pred, obj))
                sub_span = spo["subj_tok_span"]  # 表示sub的位置，例[17,24]
                obj_span = spo["obj_tok_span"]  # 表示obj的位置，例[17,24]
                pred_idx = self.pred2idx[pred]  # 关系的编号
                plus_token_pred_idx = pred_idx + token_len + 2  # tokenlen = 100
                spo_span_list.add((tuple(sub_span), pred_idx, tuple(obj_span)))  # ((17, 24), 10, (13, 16))
